...
def DrawWheel(pushUpdate=True, rOuter = 520, rInner = 400, Nspokes = 9, x=.5 ,y=.5, LoadedStimList = [], SingleStim=False):
    blank()
    # Draw Outer Circle
    ...
    # Draw Symbols
    Xminus = [55,40,50,60,50,45,40,65,55]
...
def DrawNoisyStim( StimN, LoadedList, Signal=255, mu=0, sigma=0.0625, x=.5, y=.5, UPDATE=True, scale=1 ):
    ...
    Stim[:,:,0] += noise255                 # add equal noise to 1st... Stim(1) = Stim(1) + Noise
    Stim[:,:,1] += noise255                 # 2nd ...
...
def Feedback( block, mRT, mAcc ):
    ...
    blank()
    PlotMean( mAcc, block, False, xpos=.5, ypos=.35 ) #Acc
...
def BlockOnePreamble():
    blank()
    write('You are about to start the experimental blocks,', .5, .4, UPDATE=False)
    write('trial-by-trial feedback will no longer be provided.', .5, .45, UPDATE=False)
    write('This task is designed to be difficult.', .5, .55, UPDATE=False)
    write('Do not be discouraged if you find the symbols hard to categorise.', .5, .6, UPDATE=False)
    write('Remember, if you are unsure of the correct symbol, make your best guess.', .5, .65, UPDATE=False)
    write('Press space to begin.', .5, .85, UPDATE=True)
    while True:
        for event in pygame.event.get():
