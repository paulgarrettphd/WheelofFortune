...
def PlotMean( List, BlockNumber, RTplot=True, FigSize=6, xpos=.5, ypos=.5, UPDATE=False ):
    ...
    if RTplot == False:
        ...
        ax.set_yticklabels(['Min', 'Max'], rotation=45)
        plt.title("Avg Accuracy (%)", fontsize=20)
    else:
...
def PlotBlockBar( BlockNumber, FigSize=3, xpos=.5, ypos=.5, UPDATE=False ):
    ...
    surf = pygame.image.fromstring(raw_data, size, "RGB")
    screen = DISPLAYSURF
    screen.blit( pygame.transform.scale( surf, (1200, 200)), (int(WINDOW_WIDTH*xpos+150 ) ,int(WINDOW_HEIGHT*ypos-150 ) ) )
