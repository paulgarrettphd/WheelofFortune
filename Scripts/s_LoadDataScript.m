...
% 08/10/2018
% Script to:
% - Check for pregenerated .mat files. If ~exist, load from CSV and save as
% .mat
% - Check for incomplete Chinese CSV files. If incomplete, identify missing
% conditional values and replace with NAN
% - Save as .mat files for rapid recall and loading in future.
...
for f = v.files
    ...
    % Check for missing data from the Taiwan Cohort's data set
    if sum(~isnan(NumData(:,1))) < 1305
        ...
        % Fill missing data while any exists
    ...
    % Save Raw Numeric and All String data for loading at analysis
    % Save File and participant IDs for recall
    switch v.Cohort
        case 'English'
            if NumData(1,1) == 1
...
% Clear variables to make calling script from analysis clean and easy.
